...
每個 base 有 46 個 exponent，共 368 隻既有次方角色。其後另有 `1²` 到 `50²` 的 50 隻平方角色；總角色數由 `TOTAL_CHARACTER_SLOTS` 即時計算為 418，不要把 418 另寫成業務邏輯常數。
...
八組次方角色收集完成後解鎖平方角色區；全部 418 個角色收集完成後，轉蛋關閉。
...
- 愛心上限：次方角色使用 exponent；平方角色使用平方底數，因此 `1²／2²` 有 1 種互動、`3²／4²` 有 2 種，依序至 `49²／50²` 有 25 種互動
...
目前共有 19 級，權威來源是 `src/lib/rewards.ts` 的 `LEVELS`。
