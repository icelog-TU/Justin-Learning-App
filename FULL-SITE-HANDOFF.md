...
每個 base 有 46 個 exponent，共 368 隻既有次方角色。其後依序另有六組數列角色：
...
總角色數由 `TOTAL_CHARACTER_SLOTS` 即時計算為 618，不要把 618 另寫成業務邏輯常數。六組定義與固定解鎖順序的唯一權威來源是 `SEQUENCE_CHARACTER_COLLECTIONS`。
...
固定解鎖順序為：八組次方角色 → 平方 → 立方 → 三角數 → 斐波那契 → 質數 → 階乘。全部 618 個角色收集完成後，轉蛋關閉。
...
目前共有 35 級，權威來源是 `src/lib/rewards.ts` 的 `LEVELS`。既有 Lv.1～Lv.25 門檻完全不動；四組新角色只在後面新增 Lv.26～Lv.35，最高級門檻為 618。
