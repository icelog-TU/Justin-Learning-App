...
每個 base 有 46 個 exponent，共 368 隻既有次方角色。其後依序另有 `1²` 到 `50²` 的 50 隻平方角色，以及 `1³` 到 `50³` 的 50 隻立方角色；總角色數由 `TOTAL_CHARACTER_SLOTS` 即時計算為 468，不要把 468 另寫成業務邏輯常數。
...
八組次方角色收集完成後解鎖平方角色區；平方角色完成後解鎖立方角色區；全部 468 個角色收集完成後，轉蛋關閉。
...
- 愛心上限：既有次方角色使用 exponent；平方與立方角色使用各自底數。因此 `1²／2²` 與 `1³／2³` 各有 1 種互動，`3²／4²` 與 `3³／4³` 各有 2 種，依序至 `49／50` 底數各有 25 種互動
...
- 每隻角色的互動順序固定先「打招呼」、再「聊聊天」（四顆愛心的數字教學），第三個起才依角色做可重現的隨機排列；前兩個模板不得一起被打亂。
- 同一隻角色內的互動圖示不得重複；次方、平方、立方角色的頭像統一為簡潔填色幾何風格，依序使用圓形、正方形、三角形，並沿用各自的彩虹漸變色。
- 第二個互動固定使用數字「聊聊天」模板，依該格解鎖所需的愛心數說明底數重複相乘的結果（46 顆愛心角色的第二格為 4 顆愛心，例如 2 重複 4 次得到 16）；其他互動維持各角色原有的確定性活動與不重複規則。

`AssociationCharacterDetailPage.tsx` 是「一字成語王」的真實漢字破解紀錄，不是抽卡角色頁。

### 等級

目前共有 25 級，權威來源是 `src/lib/rewards.ts` 的 `LEVELS`。既有 Lv.1～Lv.22 門檻不可向後移動；立方角色只在後面新增 Lv.23～Lv.25。
