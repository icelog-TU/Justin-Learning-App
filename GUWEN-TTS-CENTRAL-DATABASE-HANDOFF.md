...
3. 使用者在孩子實際使用的裝置上，一句一句播放並親耳判斷 TTS 是否把每個目標念對。GPT、Codex、程式測試與瀏覽器自動化不能聽見實際發音，絕對不能代替使用者標記結果。
4. 使用者按「念對」：該 exact utterance 不在孩子端加注音。
5. 使用者按「念錯」：孩子端每個受影響的朗讀位置，都在該完整整句正下方另加一行正確讀音提示；原句本身保持不變。
...
11. 同步更新：
    ...
    - 教材主檔實聽表
    - 孩子端提示或 TTS 修音
12. 念對者不顯示注音；念錯者在每個受影響的孩子端朗讀位置，緊接完整整句另加一行正確讀音提示。
...
中央資料庫、自動回傳、正式候選檔、網頁讀回與 Codex pull 指令已經存在，並已在 GitHub 固定分支上。第三篇七個語音單元仍保留舊版實聽歷史，但在 schema v2 下目前都是「待複驗」，尚不能形成「念對／不需加註」結論。
