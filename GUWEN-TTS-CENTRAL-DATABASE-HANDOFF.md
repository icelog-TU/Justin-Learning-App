...
2. Agent 先合併完全相同句子，再依「字形＋指定讀音＋影響讀音的詞義／詞組用法＋相同 `ttsSafe()`／voice 條件」分組，每組只留一個代表 target，並以最少完整代表句覆蓋所有群組後批次部署。
...
資料庫可以在同一已核准讀音群組內沿用代表結果，但不能跨越不同讀音、詞義／詞組、`ttsSafe()` 行為或 voice 條件。
...
2. 每題完成時先掃描播放白名單，不只掃古文原文，也不納入詳解、成人／出處或純顯示文字。
3. 全篇白名單完成後重新掃描其中全部多音字；不得按 agent 猜測是否容易念錯而漏字。
4. 先合併完全相同句子，再按字形、指定讀音、讀音相關用法及相同 TTS／voice 條件分組，以最少代表句覆蓋全部群組；同句多群組放入同一 item 的 `targets[]`。
...
中央資料庫、自動回傳、正式候選檔、網頁讀回與 Codex pull 指令已經存在，並已在 GitHub 固定分支上。正式實聽台目前只保留第一篇《王戎不取道旁李》；第三篇舊測試 catalog、瀏覽器清單與中央文件均已退役。
