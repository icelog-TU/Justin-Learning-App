...
2. A random line from `PRAISE_LINES` is spoken (`speak(praiseLine)` — fire-and-forget, **not** gated on its `onend`; see the pitfall below for why).
3. Simultaneously, a `celebration` state (`{ wordId, tick, stage: 'rolling' }`) starts ticking up via `tickCelebration` — a chain of `window.setTimeout`s (`CELEBRATION_TICKS` steps × `CELEBRATION_TICK_MS`, ~1.4s total), each step bumping `tick` and playing `playRollTickSound()`. The displayed coin/star numbers are `Math.round((tick / CELEBRATION_TICKS) * COIN_PER_GUWEN_WORD)` etc. — this is why the underlying reward amount doesn't need to be threaded through the roll separately, it's just a ratio of the same constant.
4. Once `tick` reaches `CELEBRATION_TICKS`, a transition effect fires the settle sound (`playCoinSound()` + a delayed `playStarSound()`), flips `stage` to `'settled'`, and the "🎉 哇，得到 X 金幣、Y 星星！" line appears. This holds for a fixed 700ms beat (a deliberate pause, not a duration guess — see the "never guess a duration" rule, which is about *external* content like TTS length, not an animation's own timing).
...
**Pausable, and always skippable.** A dedicated "⏸ 暫停播放" button on the celebration card (`toggleCelebrationPause`) pauses *both* halves at once — the praise-line TTS (`pauseSpeech()`) and the roll ticker (clears the pending tick timeout instead of scheduling the next one; resuming re-enters `tickCelebration` from the current `tick`). This needed to be bespoke rather than reusing `togglePlayback`, because `togglePlayback` only knows how to pause speech — it has no concept of a visual timer running alongside it. Separately, and just as important: **"下一個古文字" is rendered unconditionally whenever `feedback === 'correct'`, celebration or not** — it is never hidden behind the celebration card, so a child who doesn't want to wait can tap it immediately and jump straight to the next word mid-roll, no need to pause first. `stopPuzzleSpeech()` (already called at the top of `handleNextWord`/`handleResetProgress`) clears the roll's pending timeout and the celebration state along with everything else it already cleared, so skipping is always instant and clean — verified by clicking "下一個古文字" ~150ms into a fresh celebration and confirming no leftover celebration/explanation audio bleeds into the next word's puzzle.

**Pitfall avoided: don't gate the sequence on `speak()`'s `onend`.** The first version made the transition to `'settled'` wait for *both* the roll finishing *and* the praise line's `onend` firing (`praiseDone` flag). This surfaced immediately in Playwright verification: `utterance.onend` never fired in headless Chromium (no real TTS voices installed), so `praiseDone` stayed `false` forever and the celebration hung in `'rolling'` indefinitely — explanation never scheduled. Whether or not that exact failure mode happens on a real device, gating forward *progress* on a speech-completion event is fragile by nature (voices still loading, an OS-level audio hiccup, etc.), and this codebase already relies on `speak()`'s built-in `cancel()`-on-next-call behavior to prevent audio overlap everywhere else. So the fix — and the rule for any future multi-source celebration/animation — is: **let a fixed, deterministic timer (the roll's own tick count) drive stage transitions, and treat any accompanying speech as best-effort.** If the speech is still talking when the next stage's own `speak()` call fires, it gets cleanly cut off for free.
