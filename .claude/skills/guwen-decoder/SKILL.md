...
- **A 3-screen `closingSequence`** (`SequenceOrderingClosing` → `CausalChainClosing` → `EvidenceMultiSelectClosing`, all under `GuwenLesson.closingSequence`, optional): shown after every `steps` entry is solved, before the final translation unlocks. Not modeled as more `LessonStep`s — their shapes (reorder cards, a read-only summary, multi-select checkboxes) don't fit the single-correct-index quiz shape at all. Sequence ordering uses **up/down reorder buttons, not drag-and-drop** — decided with the user specifically for mobile reliability (a child's finger-drag is much easier to get wrong than two taps). Each of the 3 screens gets its own id, added to `decodedWordIds` on solve, so a reload mid-closing-sequence resumes at the right one (`closingStage` state, initialized once from `solvedIds` at mount, then advanced only by each screen's own explicit continue click — never derived live).
- **`GuwenProgress`/reward plumbing already generalized cleanly**: `allStepsSolved` from the earlier blank-page fix (see below) became `readyForComplete = allStepsSolved && allClosingSolved`, and a lesson with no `closingSequence` (司馬光) gets `allClosingSolved = true` vacuously — zero behavior change for existing lessons.
...
| `src/data/guwenLesson.ts` | **Evidence-lesson format.** `GuwenLesson`, `LessonStep` (discriminated union: `evidence`/`reconstruction`/`local_inference`/`story_reasoning`/`reveal`), `ClassicalClue`, `DecodingKey`, `FinalVerification`, `ClosingSequence` (`SequenceOrderingClosing`/`CausalChainClosing`/`EvidenceMultiSelectClosing`) interfaces + the actual lesson data (`simaGuangLesson`, `keZhouQiuJianLesson`, `guwenLessons` array) |
...
The user separately asked what happens on a full redo (reset + replay) — currently resetting keeps earned coins/stars but wipes `decodedWordIds`/`completedAt`, so nothing was stopping a full-reward redo loop before this was addressed. Decision made: a redo should still pay out, at `GUWEN_REDO_REWARD_MULTIPLIER = 0.6` (60%) of both the per-word rate and the completion bonus — enough to make replaying worthwhile without matching the first clear. Implementation:
- `GuwenProgress` gained `timesCompleted?: number`, incremented by `recordGuwenTextCompleted` and — critically — **preserved by `resetGuwenProgress`**, which is the whole point: it's the one field that has to survive a reset to tell a redo apart from a first attempt. `resetGuwenProgress` now resets to `{ decodedWordIds: [], timesCompleted: prev.timesCompleted ?? 0 }` instead of deleting the progress entry outright.
- Each page (`GuwenDecode.tsx`, `GuwenLessonDecode.tsx`) derives `isRedo` and scales `guwenCoinAmount`/`guwenStarAmount`/`completionCoinBonus`/`completionStarBonus` off it, then uses those — never the raw constants — in every `reward()` call *and* every on-screen number (progress-bar total, celebration roll-up, completion screen, the "全部破解完成再加碼"/reset-confirmation preview text).
- **`isRedo` must be captured once per run (`useState(() => (progress?.timesCompleted ?? 0) > 0)`), not derived fresh every render, and only re-set explicitly inside `handleResetProgress`.** First implementation derived it live from `progress?.timesCompleted` on every render — which broke the very first completion it was meant to celebrate: `completeGuwenText` bumps `timesCompleted` to 1 as part of *finishing* the first run, so a reactive `isRedo` flips true the instant that landing state change is reflected, and the just-earned completion screen then displays (though does not actually re-charge) the discounted redo numbers for what was genuinely a first clear. Caught via a Playwright check that compared a live first-time completion (expected 320/160 for the Sima Guang lesson) against a live redo (expected 188/104) — both showed 188/104 before this fix. The reset handler is the only place allowed to update it mid-session, since resetting is the one moment a "was this completed before *this* attempt" answer can legitimately change.
