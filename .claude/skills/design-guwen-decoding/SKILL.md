...
- Treat polyphonic-character scanning as candidate discovery, not permission to display Zhuyin. Scan each question while authoring, then perform a separate exhaustive whole-lesson scan after all child-facing spoken copy is complete. Batch-add every occurrence of every polyphonic character in every exact playable utterance to `src/data/guwenPronunciationAudit.ts`; do not filter to characters an agent predicts may be mispronounced. Deploy and open `/#/tts-audit` so the formal batch appears there; temporary pasted `待分類` lines are not formal records. Only the user can listen on the child's real device and click `念對／念錯` for each target occurrence; an agent or automated test must never infer audible correctness. A sentence with several targets is played once, but every target is judged separately and the result is submitted only after all are selected. Require a successful central-database receipt and pull current target-level results with `npm run tts:audit:pull` before editing cues. A confirmed-correct target gets no cue. Every affected occurrence of a confirmed-incorrect target gets a separate cue immediately below its complete sentence: for a non-target use, `度，當作「測量」時，念作墮（ㄉㄨㄛˋ）。`; for an unresolved target, `度，這裡念作墮（ㄉㄨㄛˋ）。` Keep Zhuyin visible, but prepare TTS text that removes the parenthesized Zhuyin and reads only the Chinese-character portion. Retest when the full utterance, TTS input, target position, revision, or applicable voice environment changes; database-wide character statistics do not create a cross-sentence exemption unless the user later approves a separately validated skip policy.
- Use `GUWEN-POLYPHONIC-AUDIT-RUNBOOK.md` as the reusable conversation entry point for this process. A finalized-lesson request triggers stage one; a user statement that all listening is complete triggers stage two. Recognize equivalent natural wording and never ask the user to paste the long workflow into every lesson conversation.
- **Never pre-reveal the answer shape before the question.** This has recurred three times across this lesson, so treat it as a standing rule, not a one-off fix:
  ...
  - **A fifth recurrence, in a different lesson (揠苗助長's `yu` step, 予), of a variant not yet catalogued: narrating the clues' underlying relationship as an established fact inside `question` itself, rather than leaking a literal answer word.** The question read "第一條是孔子在說自己的想法；第二條是孟子在說自己的理由。兩句中的「予」都指向誰？" — this doesn't quote the correct option's wording anywhere, so it would have passed every grep-based check above, but it still states as fact that both speakers are "talking about their own [thoughts/reasons]," which *is* the answer (予 refers to the speaker themself) dressed up as scene-setting narration instead of an evidence description. Compare this to acceptable clue-content restatements elsewhere in the same lesson (e.g. 病's question: "第一條線索中的百姓忙得沒有時間休息，後來「病且怠」" — describes what's *visibly in the clue text*, not the grammatical relationship being tested) — the difference is whether the restated fact is observable surface content (safe) or the very relationship/role the question asks the child to identify (a leak, even with paraphrased wording). The user caught it directly via screenshot: "這一段洩題了" and supplied the fix themselves — a neutral "讀完這兩份線索，你覺得「予」最可能是指向誰？" that asks the same question with zero pre-narrated interpretation. **Added check, alongside the two from the fourth recurrence above: does the question narrate WHO each clue's key phrase refers to, or WHAT role/relationship it plays — using any wording, not just the option's literal words? If so, it's answering the question before asking it.**
  - **A sixth recurrence, same session, same lesson (揠苗助長's `qu` step, 趨): a scene-setting parenthetical smuggling in the answer's *manner*, not its literal wording.** Clue 1's gloss read "（一群不聽指揮的士兵看見東西，便和搶栗子的猴子一樣亂成一團。）他們一看見東西，就爭著「趨」過去。" — the bracketed comparison to monkeys scrambling for chestnuts ("亂成一團") doesn't quote any option's text, but it vividly acts out exactly the answer's manner (fast, chaotic, rushing) before the child compares anything. This is a new variant again: not a literal word match, not a narrated relationship/role (the fifth recurrence's shape) — here a *sensory image* pre-enacts the correct option's physical quality. The `question` field had the same problem one level down: "一群不聽指揮的士兵爭著搶東西" used "搶" (to snatch/grab), which itself already implies fast, urgent motion — the very thing "加快腳步趕過去或離開" is supposed to be inferred as. The user's fix, given directly via screenshot with an explicit instruction ("不要寫跟猴子一樣亂成一團，暗示過多"): drop the monkey comparison entirely, keep the clue's actual subject ("一群不聽指揮的士兵") in the main clause instead of a parenthetical, describe only what they're looking at ("看見想要的東西") with no manner/speed language, and in `question`/`retryHint` replace "搶東西" with the target character itself quoted in place ("爭著「趨」想要的東西" / "「趨」而避開孔子") — same "keep the target quoted, translate around it, never paraphrase with a manner-loaded synonym" discipline used everywhere else, just applied to a verb-of-motion this time instead of a pronoun/particle. **Added check, alongside the two above: does any pre-explanation field (parenthetical context, gloss, question, or retryHint) contain a vivid scene, comparison, or manner-word (慌亂/急忙/猛然/亂成一團/搶 etc.) that already acts out the correct option's *quality of motion or emotion*, even without naming it directly? If so, replace it with a neutral description of only what's observable (who, what object, what the clue literally says), and keep the target character quoted rather than substituted with a synonym that carries the same implication.**
