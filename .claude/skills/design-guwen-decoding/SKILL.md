...
- Treat polyphonic scanning as whitelist-only candidate discovery, not permission to display Zhuyin. Scan mission opening, lesson text, App guidance, classical clues, decoded vernacular, task/question, options, correct feedback, and incorrect hints; exclude explanations, adult/source material, evidence/story/display/completion copy, and anything not played. Inside that whitelist, find every polyphonic character without risk filtering. Merge exact duplicate strings, group by character + intended reading + pronunciation-relevant use + matching `ttsSafe()`/voice conditions, and keep one representative target per group using the fewest complete representative sentences. Different readings, uses, or TTS conditions must remain separate. Only the user may click `念對／念錯`; the representative result applies to all occurrences in its approved group. Pull central results before editing cues, add cues only for confirmed-incorrect groups, and retest when group-defining conditions change.
