...
- Treat polyphonic-character scanning as candidate discovery, not permission to display Zhuyin. Add each exact utterance to `src/data/guwenPronunciationAudit.ts`, test it through `/#/tts-audit`, require a successful central-database receipt, and pull current results with `npm run tts:audit:pull` before editing the lesson. A confirmed-correct utterance gets no cue. Only a confirmed-incorrect utterance gets a nearby cue: for a non-target use, `度，當作「測量」時，念作墮（ㄉㄨㄛˋ）。`; for an unresolved target, `度，這裡念作墮（ㄉㄨㄛˋ）。` Keep Zhuyin visible, but prepare TTS text that removes the parenthesized Zhuyin and reads only the Chinese-character portion. Retest when the full utterance or TTS input changes.
