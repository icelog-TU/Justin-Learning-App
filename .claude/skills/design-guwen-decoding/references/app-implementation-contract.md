...
4. `solved`：先以獨立的「核心解答」區塊顯示並自動播放 `correctFeedback` 第一段；該區塊此後持續保留。播放完成後才在其下方發放本題獎勵並播放獎勵動畫；動畫結束後，再在核心解答下方新增獨立的「詳解」區塊，只承載並播放其餘 `correctFeedback` 段落與 `explanation`，最後才顯示密碼鑰匙及下一題按鈕。
...
- 未完成先備題時，後續題目保持鎖定；
- 答錯只顯示提示，不直接公布正解；
- 答對後先完整播放獨立的核心解答區塊；播放結束前不得出現獎勵動畫或下一題按鈕。核心解答在獎勵期間及之後都不得消失；獎勵結束後，詳解必須以第二個獨立區塊接在其下方，語音只從詳解內容開始，不重播核心解答；
- 密碼鑰匙只在對應步驟完成後取得；
- 返回課程時能恢復已完成進度；
- 全文白話文在全部必要步驟完成前不可見；
